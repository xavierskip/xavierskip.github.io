...
当年利用代码托管平台来实现静态网页资源的托管及访问，当年还是使用的 svn 做版本控制。如今使用 git 作为版本控制已经成了标配，google code 已经关闭，GitHub 一家独大。当年，静态网页生成器还不流行，更没有各种前端构建工具及云端托管平台，比如 [Vercel](https://vercel.com/)、[GitHub Pages](https://pages.github.com/)。很多东西都变了，但是需求没有变。

我发现了一个工具，就很直接的提供这个功能，从2013年到现在。

[rawgit.hack](https://raw.githack.com/)

从源代码托管服务中提供文件，并确保正确的Content-Type头部设置。还是这么直接，不需要其他工具，不需要服务托管。直接访问文件，提供正确的Content-Type，让浏览器正确识别文件页面。
